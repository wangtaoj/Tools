...
public class JsonUtil {
	...
	static {
		...
		//设置转换的日期格式, 默认是将时间转换为时间戳(老的时间类)
		objectMapper.setDateFormat(new SimpleDateFormat(pattern));

        /*
            支持jsr310, 即新的时间类(java.time包下的时间类)
            LocalDateTime序列化格式:2018-06-06T00:53:53.316(与toString方法)
            在LocalDateTime字段上使用@JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
            指定格式
         */
